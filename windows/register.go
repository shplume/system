...
func (c *Controller) GetRegisterPage() *fyne.Window {
	page := c.App.NewWindow("注册")
	page.Resize(fyne.NewSize(400, 200))

	infoCon := container.New(layout.NewCenterLayout(), widget.NewLabel("学生帐号注册\n"))
	...
	success := dialog.NewInformation("成功", "学生帐号注册成功", page)
	...
	form := &widget.Form{
		...
		OnSubmit: func() {
			...
			if passEntry.Text != confirmEntry.Text {
				information := dialog.NewInformation("注册失败", "两次密码输入不一致", page)
				information.Show()
				return
			}

			id, _ := strconv.Atoi(nameEntry.Text)
			err := db.InsertUser(c.DB, id, passEntry.Text)
			if err != nil {
				information := dialog.NewInformation("注册失败", err.Error(), page)
				information.Show()
				return
			}
